  ...
  if (await safeExists(manifestPath)) {
    try {
      ...
      for (const mod of manifest.modules) {
        ...
        const brainPath = mod.brain ? path.join(contextDir, mod.name, mod.brain) : null;
        const todoPath = mod.todo ? path.join(contextDir, mod.name, mod.todo) : null;

        consolidated[modKey] = {
          ...(mod.brain
            ? {
                [mod.brain]:
                  brainPath && (await safeExists(brainPath))
          ...
          ...(mod.todo
            ? {
                [mod.todo]:
                  todoPath && (await safeExists(todoPath))
