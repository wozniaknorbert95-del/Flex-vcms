  ...
  messages: Joi.array()
    .items(
      Joi.object({
        ...
        parts: Joi.array()
          .items(Joi.object({ text: Joi.string().max(8000).required() }))
          .optional(),
...
// --- Helpers ---
const isWindowsPath = (p) =>
  typeof p === 'string' && (/^[A-Za-z]:[/\\]/.test(p) || p.includes('\\'));
  ...
  try {
    ...
    if (hasWindowsPaths) {
      return res.status(400).json({
        error:
          'Scan aborted: repos.yaml contains local Windows paths not portable to this environment. Run scan from your laptop.',
...
router.get('/v1/context/health', readLimiter, async (req, res) => {
  try {
    ...
    if (!fsSync.existsSync(manifestPath)) {
      return res.json({
        modules: [{ name: 'deploy-context', status: 'missing' }],
